...
function buildWhatsappAppLink(phone, message) {
...
export async function shareOrderPaymentRequest({
  ...
  const directWhatsappUrl = buildWhatsappAppLink(phone, message);
  const shouldPreferDirectApp = isMobileLikeDevice();

  if (shouldPreferDirectApp) {
    const targetWindow = popupWindow && !popupWindow.closed ? popupWindow : window;

    targetWindow.location.replace(directWhatsappUrl);
